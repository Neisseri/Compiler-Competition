...
//     std::cout << "allocate reg for " << print_reg(temp) << "\n";
...
//                 if (auto ret = dynamic_cast<Return*>(it->get())) { // epilogue for all bbs that end with return
//                     BasicBlock::add_edge(bb, exit);
//                     auto inst = it->get();
//                     inst = new Branch(Reg(General, x0), exit); // unconditional branch
//                 }
//         }
//     }
